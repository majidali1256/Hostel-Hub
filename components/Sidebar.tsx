...
const Sidebar: React.FC<SidebarProps> = ({ user, unreadMessagesCount = 0, onLogout, onNavigate, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50" onClick={onClose}>
            <aside
                className="w-72 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col p-4 h-screen shadow-2xl"
            ...
            >
                ...
                <div className="flex-1 space-y-2 overflow-y-auto">
                    <button onClick={() => onNavigate('dashboard')} className={navItemClasses}>
                    ...
                    <button onClick={() => onNavigate('chat')} className={navItemClasses}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    <button onClick={() => onNavigate('agreements')} className={navItemClasses}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    {user.role === 'admin' && (
                        <button onClick={() => onNavigate('admin')} className={navItemClasses}>
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    <button onClick={() => onNavigate('profile')} className={navItemClasses}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-4">
                    ...
                    <div className="flex items-center gap-3 w-full text-left p-2 rounded-lg bg-gray-50 dark:bg-gray-900">
                        {user.profilePicture ? (
                            <img src={user.profilePicture} alt={user.username} className="w-10 h-10 rounded-full object-cover flex-shrink-0" />
                        ) : (
                            <div className="w-10 h-10 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center font-bold text-gray-600 dark:text-gray-300 flex-shrink-0">
